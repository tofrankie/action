...
根据 `tag` 自动创建或更新 GitHub Release，并可按需发布 npm。同时提供交互式 CLI 用于本地手动发布与重放。

## 提供了什么

- 同时支持单包、多包仓库
- 从 `CHANGELOG.md` 自动提取本次发布的标题与正文
- Release 幂等：同一 `tag` 重复执行时更新而不是重复创建
...
监听 tag 推送，自动创建 Release，不发布 npm。

> 若该 `tag` 已存在 Release，则会更新该 Release（幂等行为）。
...
- 先按最简示例配置（不额外传参），跑通“自动创建/更新 Release”
- 需要手动重跑某个版本时，再使用 `workflow_dispatch` 传 `tag`
- 需要同时发 npm 时，再开启 `publish-npm` 并提供 `npm-token`

常见场景推荐配置：

- 仅发布 GitHub Release：无需额外参数，参考最简示例
- 手动重跑指定版本：`tag`
- 发布 GitHub Release + 发布 npm 包：`publish-npm=true` + `npm-token`

### 为什么建议同时开启 `workflow_dispatch`

发布流程常见失败并不一定来自代码本身，例如网络抖动、权限不足、token 失效、changelog 条目不匹配。  
这些问题修复后，通常需要手动再执行一次同一版本发布，因此建议在 `push tags` 之外开启 `workflow_dispatch`。

### 进阶工作流示例（支持重放）

> 重放：不重新创建 tag，而是对同一 `tag` 的发布流程手动再执行一次（常用于首次失败后的补跑）。
...
- `npm-token`：仅 `publish-npm=true` 时必需
...
- `tag`（建议必填）：要发布的版本 tag，例如 `v1.2.3`、`pkg-a@1.2.3`
...
2. 定位并读取对应 `CHANGELOG.md` 条目，生成 release 标题与正文
3. 创建或更新同名 GitHub Release（幂等）
4. 若开启 `publish-npm`，先检查版本是否存在，再决定发布或跳过
5. 输出 `release-url`、`npm-status`、`resolved-package-name`、`resolved-version`

## 常见失败原因

- `permissions.contents` 不是 `write`，导致无法创建或更新 Release
- `tag` 格式不符合规则（尤其是 monorepo 使用了 `v1.2.3` 这类不带包名格式）
- `CHANGELOG.md` 缺少对应版本条目，或标题与当前 `tag`/版本无法匹配
- 开启了 `publish-npm` 但没有提供有效的 `npm-token`
...
- `--publish-npm`：开启 npm 发布
- `--npm-token <token>`：npm Token
- `--yes`：跳过确认

> Token 读取优先级：`--token` -> 环境变量 `GITHUB_RELEASE_TOKEN` -> 环境变量 `GITHUB_TOKEN`
...
# 同时发布 npm
$ pnpx @tofrankie/action tfr --tag v1.2.3 --publish-npm --npm-token <npm_token>
