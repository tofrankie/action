...
async function readRootPackage(rootDir) {
...
async function scanPublishablePackages(rootDir) {
	const [rootPackage, workspacePackages] = await Promise.all([readRootPackage(rootDir), scanWorkspacePackages(rootDir)]);
	if (!rootPackage) return workspacePackages;
	return workspacePackages.some((item) => item.name === rootPackage.name) ? workspacePackages : [rootPackage, ...workspacePackages];
}
